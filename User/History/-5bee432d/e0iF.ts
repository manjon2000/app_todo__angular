...
export const _todoReducer = createReducer(initialState,
    ...
    on(action.allTodoCompleted, (state => {
      
      const allTodoCompleted = state.map(x => {
        ...x,  // Copia todas las propiedades existentes del objeto todo
        isCompleted: true  // Sobrescribe la propiedad isCompleted a true
      });
      return {...state, todo: allTodoCompleted};
    })),
);
